...
The מגן אברהם (הובא ברעק"א שם) agrees with the ש"ך that the milk does not need ששים, but only when the meat did not enter the milk as part of an uninterrupted stream (עירוי שנפסק הקילוח), and so even the קליפה transfer is only at the level of a [[בישול - Cooking#כלי שני|כלי שני]] and is not actually אסור. In such a case, removing the קליפה from the meat is only a חומרא due to the optics of eating the portion of the meat that came into contact with milk. Such a concern doesn't apply to the milk, because there is no distinct קליפה in fluid.

This is only a , however, because we are discussing a scenario in which  because it is only a ^[Because we are dealing with a case in which the meat did not enter the milk as part of an uninterrupted stream] and so doesn't really cook
##### Pan-Frying:

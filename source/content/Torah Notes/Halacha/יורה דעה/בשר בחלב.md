...
The רמ"א (שם) rules that we should be stringent like this opinion initially (לכתחילה), and therefore, in a case of Meat>Pot>Fish for example, not combine the fish with dairy, but that if one already combined the fish with dairy (fried it in butter, for example) one is allowed to eat it (בדיעבד). In a case of Meat>Plate>Fish, one could even combine the fish with dairy לכתחילה, because even the stringent opinion cited by the רמ"א agrees that such a case is נב"נ.
### Dehydrated Meat:

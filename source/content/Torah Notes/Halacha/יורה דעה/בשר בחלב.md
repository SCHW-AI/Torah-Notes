...
+ If the knife contained flavor of meat/milk, the above would only be true if the blade had a greater volume than the radish, so there would be enough בליעות in the knife to permeate all   of the radish. Otherwise, if one cooked the whole radish with dairy, one would only need 60 times the blade; if one combined a piece with dairy...
+ If the knife contai